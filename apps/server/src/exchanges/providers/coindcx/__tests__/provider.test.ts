import { beforeEach, describe, expect, it, vi } from 'vitest';
import pino from 'pino';
import { CoinDCXProvider } from '../provider.js';
import type { RestClientTransport } from '../../../transport/rest-client.js';
import type { CoinDCXMarket, CoinDCXTicker, CoinDCXFee } from '../types.js';

class MockTransport implements RestClientTransport {
  request = vi.fn();
}

describe('CoinDCXProvider', () => {
  let transport: MockTransport;
  let provider: CoinDCXProvider;
  let logger: pino.Logger;

  const markets: CoinDCXMarket[] = [
    {
      symbol: 'BTC/INR',
      baseAsset: 'BTC',
      quoteAsset: 'INR',
      minQty: '0.001',
      tickSize: '0.01',
      isActive: true,
    },
  ];

  const tickers: CoinDCXTicker[] = [
    {
      market: 'BTC/INR',
      lastPrice: '5000000',
      volume: '10.5',
      high: '5100000',
      low: '4900000',
      change: '0.02',
      bid: '4999000',
      ask: '5001000',
    },
  ];

  const fees: CoinDCXFee[] = [{ symbol: 'BTC/INR', makerFee: '0.001', takerFee: '0.002' }];

  beforeEach(() => {
    transport = new MockTransport();
    logger = pino({ level: 'silent' });
    provider = new CoinDCXProvider(logger, transport as unknown as RestClientTransport, {
      name: 'COINDCX',
      timeoutMs: 1000,
      maxRetries: 1,
      enableWebSocket: false,
    });
  });

  it('connects and loads exchange info', async () => {
    vi.mocked(transport.request)
      .mockResolvedValueOnce({ data: markets, status: 200 })
      .mockResolvedValueOnce({ data: markets, status: 200 });

    await provider.connect();

    const loadedMarkets = await provider.getMarkets();
    expect(loadedMarkets).toHaveLength(1);
    expect(loadedMarkets[0].symbol).toBe('BTC/INR');
  });

  it('normalizes ticker and order book responses', async () => {
    vi.mocked(transport.request)
      .mockResolvedValueOnce({ data: markets, status: 200 })
      .mockResolvedValueOnce({ data: markets, status: 200 })
      .mockResolvedValueOnce({ data: tickers, status: 200 })
      .mockResolvedValueOnce({ data: tickers, status: 200 })
      .mockResolvedValueOnce({ data: fees, status: 200 });

    await provider.connect();

    const ticker = await provider.getTicker('BTC/INR');
    const orderBook = await provider.getOrderBook('BTC/INR');
    const tradingFees = await provider.getTradingFees();

    expect(ticker.symbol).toBe('BTC/INR');
    expect(orderBook.symbol).toBe('BTC/INR');
    expect(tradingFees[0].symbol).toBe('BTC/INR');
  });
});
